...
- **Stock hat kein seller_country pro Entry (BL liefert das nicht):** deshalb crawlt der Tracker Stock pro `sellerCountries[i]` einzeln mit BL-Parameter `country_code=X` (Server-seitiger Filter). Gespeichert wird mit `seller_country=X` statt `'XX'`. Für Bestandsdaten und Fälle wo der Country-Crawl noch nicht durchgelaufen ist, matcht jede `sellerCountries`-Aggregation zusätzlich `OR seller_country = 'XX'` (Fallback). Sold-Crawl bleibt weltweit weil dort pro Entry seller/buyer-Codes vorhanden sind — SQL-Filter reicht. Umsetzung: `fetchPriceData` in `src/lib/fetch-prices.ts` bekommt optional `stockCountryCodes`, alle Caller (`computeExternalPrice`, `upsertLot`, Crawler `processStock`, Detail-Refresh-Route) reichen `sellerCountries` durch.
- **Completeness-Fallback bei Aggregation:** Alt-Sales aus der Zeit vor der Completeness-Migration sind mit `completeness=NULL` gespeichert. Watchlist mit `completeness='C'/'I'/'S'` würde diese Sales nicht matchen. `getStatsForLot()` in `lot-pricing.ts` fällt bei fehlendem exakten Match auf `completeness=NULL` zurück -- deckt auch Minifig-SETs ab wo BL den Split gar nicht implementiert.
- **Cache-Konsistenz:** `fetchPriceData` ruft am Ende `recomputeAllLotsForPart` auf. Damit ist der Watchlist-Cache (`user_watchlists.suggestedPrice`) auch nach externem-API-triggered Fetch aktuell (nicht nur nach Crawler-Worker-Run).
- **External API bevorzugt Cache bei blInventoryId-Match:** `computeExternalPrice` in `external-price.ts` liefert bei mitgeschicktem `blInventoryId` den gecachten `user_watchlists.suggestedPrice` statt LIVE zu berechnen. Grund: die LIVE-Berechnung setzt sold7d/30d/60d/90dMedian hardcoded auf 0 (nur 6M + Stock werden aggregiert) — Formeln die Zeit-Buckets nutzen bekommen sonst falsche Werte. Der Cache ist von `recomputeLotPricing` mit allen Buckets korrekt gefüllt.
- **Crawler-Priorisierung (`workers/crawler.ts`):** drei Prioritätsebenen: (1) `changedAt` (Inventar-Änderung), (2) Missstände (Country-Mismatch: neueste Stock-Snapshot enthält nur `'XX'` obwohl User `sellerCountries` gesetzt hat -> forced Stock-Job mit `country_code=X`), (3) Rotation nach `freshDays`. Budget: `0.80 * (dailyLimit - externalCalls - maintenanceCallsPerDay)` als Boost für Missstände, 20% Reserve für Spikes. `calculateDelay` und `findNextJob` implementieren das gemeinsam. Boost + Missstand-Zaehler werden im Dashboard-Stats-Endpoint gespiegelt.
...
- **Pricing formulas:** Per-user configurable pricing rules with filters (item type, condition, color, category). Safe expression parser with 26+ variables (incl. `myCost`). See `src/lib/pricing-engine.ts`. Formulas round output to 3 decimals (BrickStore/BrickLink standard). **`myCost` from BL is per-unit** (Stück-Einkaufspreis) — nicht durch `myQty` teilen! (Live-verifiziert 2026-08-16 via `getInventory`.)
...
- **BSX-Export (`/api/watchlist/export-bsx`):** 1:1 round-trip mirror of the user's BL inventory with only the recommended price differing. Field mapping is critical: `<Remarks>` gets `user_watchlists.remarks` (private storage location like `TA039`), `<Comments>` gets `description` (public BL description). `<Cost>` = `myCost` **direkt** (BL liefert bereits per-Stück, wir speichern 1:1). `<LotID>` = `blInventoryId` — BrickStore matches by this on re-upload and preserves everything else. `<DateAdded>` from `bl_date_added` (BL `date_created`). Only lots with `bl_inventory_id` are exported (safety — no round-trip anchor otherwise). Locked-price lots (`priceLocked=true`) use `myPrice` regardless of formula.
...
- `user_watchlists` -- per-user tracked parts with `bl_inventory_id`, `description` (public BL description), `remarks` (private BL storage location like `TA039`), `myPrice`, `myQuantity`, `saleRate`, `myCost` (**per-Stück-Einkaufspreis**, 1:1 wie von BL geliefert), `bulk` (BL bulk qty, default 1), `blDateAdded` (BL `date_created`), `prevQuantity`, `changedAt`, `priceLocked`, alert thresholds, **and the pricing cache: `suggestedPrice`, `suggestedRuleName`, `marketStockMedian`, `marketSoldMedian`, `trend`, `pricingComputedAt`**
